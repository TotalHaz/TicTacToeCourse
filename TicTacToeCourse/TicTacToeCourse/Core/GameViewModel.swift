...
private extension GameViewModel {
    ...
    func playMove(row: Int, col: Int) {
        do {
            ...
            if let winningCellCoordinatesPath = gameStore.checkWin(in: board, for: currentPlayer.cellSymbol) {
                Task {
                    await animateWinningcellsPath(winningCellCoordinatesPath)
    ...
    func animateWinningcellsPath(_ winningCellCoordinatesPath: [CellCoordinate]) async {
        isAnimationInProgress = true
        defer {
            isAnimationInProgress = false
        }
        winningCells = .init()
        for cellCoordinate in winningCellCoordinatesPath {
    ...
    func transistionGameState(to newState: GameState) {
    ...
    func resetGameState() {
        ...
        transistionGameState(to: .playing)
    }
