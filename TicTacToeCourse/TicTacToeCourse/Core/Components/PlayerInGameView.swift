...
private extension PlayerInGameView {
    var imageView: some View {
        Image(player.image)
            ...
            .overlay(RoundedRectangle(cornerRadius: AppCornerRadius.overall.value)
                .stroke(player.type.isBot ? Color.appTheme.alternateAccent : Color.appTheme.accent, lineWidth: 0.4)
            )
    ...
    var nameView: some View {
        ZStack(alignment: orientation == .left ? .leading : .trailing) {
            // This is purely so there will always be room for Player 1 or 2 without reshaping  the space
            Text("Player12")
                .font(.headline)
                .opacity(0)
...
fileprivate struct Preview: View {
    var body: some View {
        HStack(spacing: 0) {
            PlayerInGameView(player: .defaultPlayer1,
                             orientation: .left,
                             isCurrentPlayer: true,
                             winsCount: 3)
            Spacer(minLength: 5)
            PlayerInGameView(player: .defaultPlayer2,
                             orientation: .left,
